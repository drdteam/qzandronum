	...
	if ((dmflags2 & DF2_KILL_MONSTERS) && level.killed_monsters != level.total_monsters)
	{
		...
		if ( ( fPercentKilled < sv_killallmonsters_percentage ) && ( GAMEMODE_GetCurrentFlags() & GMF_COOPERATIVE ) )
		{
			...
			if ( self->player && self->player->mo )
			{
				...
				P_Teleport (self, DVector3 ( pSpot->pos.X, pSpot->pos.Y, ONFLOORZ ), static_cast<double> ( pSpot->angle ), TELF_SOURCEFOG | TELF_DESTFOG);
...
void P_SpawnSpecials (void)
{
	...
	for ( i = 0; i < numsectors; i++ )
	{
		...
		sectors[i].SavedFloorXOffset = sectors[i].GetXOffsetF(sector_t::floor);
		sectors[i].SavedFloorYOffset = sectors[i].GetYOffsetF(sector_t::floor,false);
		sectors[i].SavedCeilingXOffset = sectors[i].GetXOffsetF(sector_t::ceiling);
		sectors[i].SavedCeilingYOffset = sectors[i].GetYOffsetF(sector_t::ceiling,false);
		sectors[i].SavedFloorXScale = sectors[i].GetXScaleF(sector_t::floor);
		sectors[i].SavedFloorYScale = sectors[i].GetYScaleF(sector_t::floor);
		sectors[i].SavedCeilingXScale = sectors[i].GetXScaleF(sector_t::ceiling);
		sectors[i].SavedCeilingYScale = sectors[i].GetYScaleF(sector_t::ceiling);
		...
		sectors[i].SavedBaseFloorAngle = sectors[i].planes[sector_t::floor].xform.baseAngle;
		sectors[i].SavedBaseFloorYOffset = sectors[i].planes[sector_t::floor].xform.baseyOffs;
		sectors[i].SavedBaseCeilingAngle = sectors[i].planes[sector_t::ceiling].xform.baseAngle;
		sectors[i].SavedBaseCeilingYOffset = sectors[i].planes[sector_t::ceiling].xform.baseyOffs;
		...
		sectors[i].SavedDamage = sectors[i].damageamount;
		sectors[i].SavedMOD = sectors[i].damagetype;
