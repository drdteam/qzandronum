...
{
	float angleDiff = ( ( actor->Pos().XY() - ViewPos.XY() ).Angle() - actor->Angles.Yaw ).Normalized180().Degrees;
...
void gl_RenderModel(GLSprite * spr)
{
	...
	// [BB] Change the angle so that the object is exactly facing the camera in the x/y plane.
		...
		gl_RenderState.mModelMatrix.rotate( -( spr->actor->Pos().XY() - ViewPos.XY() ).Angle().Degrees, 0, 1, 0);
	...
	{
		const double distance = ( spr->actor->Pos() - ViewPos ).Length();
		const float pitch = RAD2DEG ( atan2( spr->actor->Z() - ViewPos[2], distance ) );
	...
	if ( (smf->flags & MDL_ROLLAGAINSTANGLE) )
		gl_RenderState.mModelMatrix.rotate( gl_RollAgainstAngleHelper ( spr->actor ), 1, 0, 0);
	else
