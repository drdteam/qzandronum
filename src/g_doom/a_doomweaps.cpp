...
{
	double bouncevelx = Vel.X / 4;
	double bouncevely = Vel.Y / 4;
	double bouncevelz = Vel.Z * ( -0.6 );
/*
...
DEFINE_ACTION_FUNCTION(AActor, A_FireBFG)
{
	...
	P_SpawnPlayerMissile (self,  0, 0, 0, PClass::FindActor("BFGBall"), self->Angles.Yaw, NULL, NULL, !(dmflags2 & DF2_YES_FREEAIMBFG));

	// [BC] Apply spread.
	if ( player->cheats2 & CF2_SPREAD )
	{
		P_SpawnPlayerMissile (self,  0, 0, 0, PClass::FindActor("BFGBall"), self->Angles.Yaw + 15, NULL, NULL, !(dmflags2 & DF2_YES_FREEAIMBFG), 0, false, false );
		P_SpawnPlayerMissile (self,  0, 0, 0, PClass::FindActor("BFGBall"), self->Angles.Yaw - 15, NULL, NULL, !(dmflags2 & DF2_YES_FREEAIMBFG), 0, false, false );
	}
