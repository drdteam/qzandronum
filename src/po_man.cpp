...
void DPolyAction::SetSpeed (double speed)
{
	m_Speed = speed;
}

double DPolyAction::GetDist ()
{
	return ( m_Dist );
}

void DPolyAction::SetDist (double dist)
{
	m_Dist = dist;
}
...
{
	SERVERCOMMANDS_DoMovePoly( m_Speedv, m_PolyObj, ulClient, SVCF_ONLYTHISCLIENT );
}

const DVector2& DMovePoly::GetSpeedV( void )
{
	return ( m_Speedv );
}

void DMovePoly::SetSpeedV( const DVector2 &speedV )
{
	m_Speedv = speedV;
}

DAngle DMovePoly::GetAngle ()
{
	return ( m_Angle );
}

void DMovePoly::SetAngle (DAngle angle)
{
	m_Angle = angle;
}
...
void DPolyDoor::UpdateToClient( ULONG ulClient )
{
	...
	if (m_Tics != 0)
		SERVERCOMMANDS_DoPolyDoor( m_Type, DVector2 ( 0, 0 ), 0, m_PolyObj, ulClient, SVCF_ONLYTHISCLIENT );
	else // [WS] Door is in motion, inform the client.
	{
		// [WS] Play the sound.
		SERVERCOMMANDS_PlayPolyobjSound( m_PolyObj, 0 );
		SERVERCOMMANDS_DoPolyDoor( m_Type, m_Speedv, m_Speed, m_PolyObj, ulClient, SVCF_ONLYTHISCLIENT );
	}
}

DAngle DPolyDoor::GetDirection ()
{
	return ( m_Direction );
}

void DPolyDoor::SetDirection (DAngle direction)
{
	m_Direction = direction;
}

double DPolyDoor::GetTotalDist ()
{
	return ( m_TotalDist );
}

void DPolyDoor::SetTotalDist (double dist)
{
	m_TotalDist = dist;
}
				  ...
				  double dist, bool overRide)
{
	...
	while ((poly = it.NextMirror()) != NULL)
	{
		...
		if ( NETWORK_GetState( ) == NETSTATE_SERVER )
			SERVERCOMMANDS_DoMovePoly( pe->m_Speedv, pe->m_PolyObj );
	...
	case PODOOR_SLIDE:
		...
		else
		{
			...
			{ // open back up
				...
				{
					SERVERCOMMANDS_PlayPolyobjSound( m_PolyObj, 0 );
					SERVERCOMMANDS_SetPolyDoorSpeedPosition( m_PolyObj, m_Speedv, poly->StartSpot.pos );
				}
			}
		}
		break;
...
bool EV_OpenPolyDoor(line_t *line, int polyNum, double speed, DAngle angle, int delay, double distance, podoortype_t type)
{
	...
	while ((poly = it.NextMirror()) != NULL)
	{
		...
		if ( NETWORK_GetState( ) == NETSTATE_SERVER )
		{
			SERVERCOMMANDS_DoPolyDoor( pd->m_Type, pd->m_Speedv, pd->m_Speed, pd->m_PolyObj );
...
void PO_Init (void)
{
	...
	for (polyspawn = polyspawns, prev = &polyspawns; polyspawn;)
	{
		...
		{ 
			...
			polyobjs[polyIndex].SavedStartSpot = polyobjs[polyIndex].StartSpot.pos;
