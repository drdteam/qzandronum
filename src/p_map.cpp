	...
	bool missileCheck)	// [GZ] Fired missiles ignore the drop-off test
{
	...
	if (!P_CheckPosition(thing, pos, tm) && CLIENT_CanClipMovement(thing))
	{
		AActor *BlockingMobj = thing->BlockingMobj;
		// Solid wall or thing
...
//	if (!P_OldCheckPosition (thing, x, y))
	if (!P_CheckPosition (thing, DVector2 ( FIXED2DBL ( x ), FIXED2DBL ( y ) ), tm))
...
void FSlide::OldHitSlideLine(line_t *ld)
{
	...
	* cph - DEMOSYNC - should only affect players in Boom demos? */
	...
	{ // ST_HORIZONTAL
		...
		{
			tmmove.X /= 2; // absorb half the momentum
			tmmove.Y = FIXED2DBL ( -FLOAT2FIXED ( tmmove.Y )/2 );
	...
	{ // ST_VERTICAL
		...
		{
			tmmove.X = FIXED2DBL ( -FLOAT2FIXED ( tmmove.X )/2 ); // absorb half the momentum
			tmmove.Y /= 2;
			//S_StartSound(slidemo,sfx_oof); // oooff!                      //   ^
		}                                                             //   |
		else                                                            // phares
...
void FSlide::OldSlideMove (AActor *mo)
{
	...
	do 
	{
		...
		OldSlideTraverse(leadx, leady, leadx+FLOAT2FIXED ( mo->Vel.X ), leady+FLOAT2FIXED ( mo->Vel.Y ));
		OldSlideTraverse(trailx, leady, trailx+FLOAT2FIXED ( mo->Vel.X ), leady+FLOAT2FIXED ( mo->Vel.Y ));
		OldSlideTraverse(leadx, traily, leadx+FLOAT2FIXED ( mo->Vel.X ), traily+FLOAT2FIXED ( mo->Vel.Y ));

		// move up to the wall

		if (FLOAT2FIXED ( bestSlidefrac ) == FRACUNIT+1)
		{
			// the move must have hit the middle, so stairstep
...
stairstep:
		...
		if ((bestSlidefrac -= FIXED2DBL ( 0x800 )) > 0)
		{
			fixed_t newx = FixedMul(FLOAT2FIXED ( mo->Vel.X ), FLOAT2FIXED ( bestSlidefrac ));
			fixed_t newy = FixedMul(FLOAT2FIXED ( mo->Vel.Y ), FLOAT2FIXED ( bestSlidefrac ));
		...
		bestSlidefrac = FLOAT2FIXED ( FRACUNIT-(FLOAT2FIXED ( bestSlidefrac )+0x800) );

		if (FLOAT2FIXED ( bestSlidefrac ) > FRACUNIT)
			bestSlidefrac = FIXED2DBL ( FRACUNIT );

		if (FLOAT2FIXED ( bestSlidefrac ) <= 0)
			break;

		tmmove.X = FIXED2DBL ( FixedMul(FLOAT2FIXED ( mo->Vel.X ), FLOAT2FIXED ( bestSlidefrac )) );
		tmmove.Y = FIXED2DBL ( FixedMul(FLOAT2FIXED ( mo->Vel.Y ), FLOAT2FIXED ( bestSlidefrac )) );
		...
		* cph - DEMOSYNC? */
		if (mo->player && mo->player->mo == mo)
		{
			if (abs(FLOAT2FIXED(mo->player->Vel.X)) > abs(FLOAT2FIXED ( tmmove.X )))
				mo->player->Vel.X = tmmove.X;
			if (abs(FLOAT2FIXED(mo->player->Vel.Y)) > abs(FLOAT2FIXED ( tmmove.Y )))
	...
	DAngle pitch, int damage, FName damageType, PClassActor *pufftype, int flags, FTranslatedLineTarget*victim, int *actualdamage)
{
	...
	// [Spleen]
	const bool hitSomething = Trace(t1->PosAtZ(shootz), t1->Sector, direction, distance, MF_SHOOTABLE, 
	...
	else
	{
		...
		else
		{
			...
			int newdam = damage;
			// [EP] Skip this for clients.
			if ( ( NETWORK_InClientMode( ) == false ) && (damage || (puffDefaults != NULL && ((puffDefaults->flags6 & MF6_FORCEPAIN) || (puffDefaults->flags7 & MF7_CAUSEPAIN)))))
			{
	...
	while ((it.Next(&cres)))
	{
		                       ...
		                       && !( zacompatflags & ZACOMPATF_OLDRADIUSDMG ) ) )
		{
			...
			{ // OK to damage; target is in direct path
				...
				const double origvelx = thing->Vel.X;
				const double origvely = thing->Vel.Y;
				...
				if (!(thing->flags & MF_ICECORPSE))
				{
					...
					if ((flags & RADF_NODAMAGE) || !(bombspot->flags2 & MF2_NODMGTHRUST))
					{
						if (bombsource == NULL || !(bombsource->flags2 & MF2_NODMGTHRUST))
						{
							...
							{
								thing->Vel.X = origvelx + ((thing->X() - bombspot->X()) * thrust);
								thing->Vel.Y = origvely + ((thing->Y() - bombspot->Y()) * thrust);
							}
							else
							{
