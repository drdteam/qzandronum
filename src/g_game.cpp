...
static fixed_t TeamLMSPlayersRangeFromSpot( ULONG ulPlayer, FPlayerStart *spot )
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		ulNumSpots++;
		distance += FLOAT2FIXED ( players[i].mo->Distance2D( spot->pos.X, spot->pos.Y ) );
	}
...
*/
		...
		if (( sectors[ulIdx].SavedBaseFloorAngle != sectors[ulIdx].planes[sector_t::floor].xform.baseAngle ) ||
			( sectors[ulIdx].SavedBaseFloorYOffset != sectors[ulIdx].planes[sector_t::floor].xform.baseyOffs ) ||
			( sectors[ulIdx].SavedBaseCeilingAngle != sectors[ulIdx].planes[sector_t::ceiling].xform.baseAngle ) ||
			( sectors[ulIdx].SavedBaseCeilingYOffset != sectors[ulIdx].planes[sector_t::ceiling].xform.baseyOffs ))
		{
			sectors[ulIdx].planes[sector_t::floor].xform.baseAngle = sectors[ulIdx].SavedBaseFloorAngle;
			sectors[ulIdx].planes[sector_t::floor].xform.baseyOffs = sectors[ulIdx].SavedBaseFloorYOffset;
			sectors[ulIdx].planes[sector_t::ceiling].xform.baseAngle = sectors[ulIdx].SavedBaseCeilingAngle;
			sectors[ulIdx].planes[sector_t::ceiling].xform.baseyOffs = sectors[ulIdx].SavedBaseCeilingYOffset;
	...
	for ( unsigned int i = 0; i < existingActors.Size(); ++i )
	{
		...
		if ( ( pActor->GetClass()->TypeName == NAME_PointPusher ) || ( pActor->GetClass()->TypeName == NAME_PointPuller ) )
		{
			...
			{
				pActor->SetOrigin ( pActor->SpawnPoint, false );
...
AActor* GAME_SelectRandomSpotForArtifact ( PClassActor *pArtifactType, const TArray<FPlayerStart> &Spots )
{
	...
	for (unsigned int j = 0; j < 2*Spots.Size(); ++j)
	{
		...
		pArtifact = Spawn( pArtifactType, DVector3 ( Spots[i].pos.X, Spots[i].pos.Y, ONFLOORZ ), ALLOW_REPLACE );
	...
	return Spawn( pArtifactType, DVector3 ( Spots[spotNum].pos.X, Spots[spotNum].pos.Y, ONFLOORZ ), ALLOW_REPLACE );
}
...
void G_DoLoadGame ()
{
	...
	try
	{
		...
		if (engine.CompareNoCase(GetEngineString()) != 0)
		{
...
void G_DoSaveGame (bool okForQuicksave, FString filename, const char *description)
{
	...
	savegameinfo.AddString("Software", buf)
		// [BB] Use GetEngineString() to distinguish Software from GL saves.
		.AddString("Engine", GetEngineString())
		("Save Version", ver)
	...
	if ( invasion )
		INVASION_Serialize( savegameglobals );

	// [BB] Save the netstate.
	if ( ( NETWORK_GetState( ) == NETSTATE_SINGLE ) || ( NETWORK_GetState( ) == NETSTATE_SINGLE_MULTIPLAYER ) )
	{
		BYTE multiplayerEmulation = !!( NETWORK_GetState( ) == NETSTATE_SINGLE_MULTIPLAYER );
		savegameglobals("multiplayerEmulation", multiplayerEmulation);
	}
