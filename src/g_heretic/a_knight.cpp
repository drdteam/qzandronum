...
DEFINE_ACTION_FUNCTION(AActor, A_KnightAttack)
{
	...
	{ // Red axe
		P_SpawnMissileZ (self, self->Z() + 36*FRACUNIT, self->target, PClass::FindActor("RedAxe"), true); // [BB] Inform clients
		return 0;
	}
	// Green axe
	P_SpawnMissileZ (self, self->Z() + 36*FRACUNIT, self->target, PClass::FindActor("KnightAxe"), true); // [BB] Inform clients
	return 0;
}
