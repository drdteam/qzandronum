...
fixed_t P_OldXYMovement( AActor *mo )
{
	...
	xmove = clamp( mo->_f_velx(), -maxmove, maxmove );
	ymove = clamp( mo->_f_vely(), -maxmove, maxmove );

	if (!(mo->_f_velx() | mo->_f_vely())) // Any velocity?
	{
		if (mo->flags & MF_SKULLFLY)
		{
...
*/
	...
	{ // Don't stop sliding if halfway off a step with some momentum
		if (mo->_f_velx() > FRACUNIT/4 || mo->_f_velx() < -FRACUNIT/4
			|| mo->_f_vely() > FRACUNIT/4 || mo->_f_vely() < -FRACUNIT/4)
		{
	...
	// Stop voodoo dolls that have come to rest, despite any
	// moving corresponding player:
	if (mo->_f_velx() > -STOPSPEED && mo->_f_velx() < STOPSPEED
		&& mo->_f_vely() > -STOPSPEED && mo->_f_vely() < STOPSPEED
		&& (!player || (player->mo != mo)
			|| !(player->cmd.ucmd.forwardmove | player->cmd.ucmd.sidemove)))
	{
		// if in a walking frame, stop moving
	...
	else
	{
		...
		*/
		...
		mo->Vel.X = FIXED2DBL ( FixedMul(mo->_f_velx(), friction) );
		mo->Vel.Y = FIXED2DBL ( FixedMul(mo->_f_vely(), friction) );
		...
		if (mo->Z() <= mo->floorz)
		{
			...
			if (mo->_f_velz() < 0)
			{
				...
				if (mo->player)
				{
					...
					if ( ( zacompatflags & ZACOMPATF_SKULLTAG_JUMPING ) || mo->player->jumpTics < 0 || mo->_f_velz() < minvel)
...
*/
	...
	if (!mo->player->morphTics && mo->health > 0 && NETWORK_IsConsolePlayerOrNotInClientMode( mo->player ))
	{
		...
		if (mo->player->mo && mo->health > 0 && mo->_f_velz() < -mo->player->mo->GruntSpeed)
		{
...
AActor *P_SpawnPuff (AActor *source, PClassActor *pufftype, fixed_t x, fixed_t y, fixed_t z, angle_t hitdir, angle_t particledir, int updown, int flags, AActor *vict, bool bTellClientToSpawn)
{
...
void P_BloodSplatter (fixedvec3 pos, AActor *originator)
{
	...
	if ( NETWORK_GetState( ) == NETSTATE_SERVER )
		SERVERCOMMANDS_SpawnBloodSplatter(pos.x, pos.y, pos.z, originator, false );
}
...
void P_BloodSplatter2 (fixedvec3 pos, AActor *originator)
{
	...
	// [BB] Tell the clients to spawn the splatter.
	if ( NETWORK_GetState( ) == NETSTATE_SERVER )
		SERVERCOMMANDS_SpawnBloodSplatter( pos.x, pos.y, pos.z, originator, true );
}
...
AActor *P_SpawnMissileAngle (AActor *source, PClassActor *type,
	angle_t angle, fixed_t vz, bool bSpawnOnClient) // [BB] Added bSpawnOnClient.
{
	if (source == NULL)
	{
		return NULL;
...
AActor *P_SpawnMissileAngleZ (AActor *source, fixed_t z,
	PClassActor *type, angle_t angle, fixed_t vz, bool bSpawnOnClient) // [BB] Added bSpawnOnClient.
{
...
AActor *P_SpawnMissileAngleZSpeed (AActor *source, fixed_t z,
	PClassActor *type, angle_t angle, fixed_t vz, fixed_t speed, AActor *owner, bool checkspawn, const bool bSpawnOnClient ) // [BB] Added bSpawnOnClient.
{
	if (source == NULL)
	{
		return NULL;
