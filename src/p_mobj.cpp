...
void AActor::Serialize(FSerializer &arc)
{
	...
	arc ( "ulLimitedToTeam", ulLimitedToTeam ) // [BB]
		( "lFixedColorma", lFixedColormap ) // [BB]
		( "lNetID", lNetID ) // [BC] We need to archive this so that it's restored properly when going between maps in a hub.
		( "ulSTFlags", ulSTFlags ) 
		( "ulNetworkFlags", ulNetworkFlags ) 
		( "ulInvasionWave", ulInvasionWave )
		( "pMonsterSpot", pMonsterSpot )
		( "pPickupSpot", pPickupSpot )
		( "Rune", Rune );
}
