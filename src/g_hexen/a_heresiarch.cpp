...
DEFINE_ACTION_FUNCTION(AActor, A_SorcBallOrbit)
{
	...
	case SORC_STOPPING:			// Balls stopping
			 ...
			 absangle(angle, parent->Angles.Yaw) < 42.1875)
		{
			...
			if ( NETWORK_GetState( ) == NETSTATE_SERVER )
			{
				SERVERCOMMANDS_SetThingArguments( self->target );
				// [BB] FIXME: Change SERVERCOMMANDS_SetThingSpecial1 to set BallAngle.
	...
	case SORC_FIRESPELL:			// Casting spell
		if (parent->StopBall == self->GetClass())
		{
			...
			if (parent->health > 0)
			{
				...
				if ( NETWORK_GetState( ) == NETSTATE_SERVER )
					SERVERCOMMANDS_SetThingFrame( parent, parent->FindState("Attack1") );
		...
		if (parent->StopBall == self->GetClass())
		{
			...
			if (self->special2-- <= 0)
			{
				...
				if (parent->health > 0)
				{
					// [BB] Tell clients to change this thing's state.
					if ( NETWORK_GetState( ) == NETSTATE_SERVER )
						SERVERCOMMANDS_SetThingFrame( parent, parent->FindState("Attack2") );
...
void ASorcBall::CastSorcererSpell ()
{
	...
	if (target->health > 0)
	{
		// [BB] Tell clients to change this thing's state.
		if ( NETWORK_GetState( ) == NETSTATE_SERVER )
			SERVERCOMMANDS_SetThingFrame( target, target->FindState("Attack2") );
...
void A_SorcOffense2(AActor *self)
{
	...
	if ( NETWORK_GetState( ) == NETSTATE_SERVER )
		SERVERCOMMANDS_SetThingArguments( self );
...
DEFINE_ACTION_FUNCTION(AActor, A_SorcFX2Split)
{
	...
	if (mo)
	{
		...
		if ( NETWORK_GetState( ) == NETSTATE_SERVER )
		{
			...
			SERVERCOMMANDS_SetThingFrame( mo, mo->FindState("Orbit") );
		}
