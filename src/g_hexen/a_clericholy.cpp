...
DEFINE_ACTION_FUNCTION(AActor, A_CHolyAttack)
{
	...
	{
		missile = P_SpawnPlayerMissile (self, 0,0,0, PClass::FindActor ("HolyMissile"), self->Angles.Yaw + 15, &t);
		if (missile != NULL && !t.unlinked)
		{
			missile->tracer = t.linetarget;
		}
...
DEFINE_ACTION_FUNCTION(AActor, A_ClericAttack)
{
	...
	AActor * missile = P_SpawnMissileZ (self, self->_f_Z() + 40*FRACUNIT, self->target, PClass::FindActor ("HolyMissile"), true); // [BB] Inform clients
