...
DEFINE_ACTION_FUNCTION(AActor, A_IceGuyAttack)
{
	...
	P_SpawnMissileXYZ(self->_f_Vec3Angle(self->_f_radius()>>1, self->_f_angle()+ANG90, 40*FRACUNIT), self, self->target, PClass::FindActor ("IceGuyFX"), true, NULL, true);	// [BB] Inform the clients.
	P_SpawnMissileXYZ(self->_f_Vec3Angle(self->_f_radius()>>1, self->_f_angle()-ANG90, 40*FRACUNIT), self, self->target, PClass::FindActor ("IceGuyFX"), true, NULL, true);	// [BB] Inform the clients.
