...
void DScroller::UpdateToClient( ULONG ulClient )
{
	...
	case EScroll::sc_carry_ceiling:
		...
		SERVERCOMMANDS_DoScroller( static_cast<int> ( m_Type ), m_dx, m_dy, m_Affectee, ulClient, SVCF_ONLYTHISCLIENT );
		break;
	}
}
...
void SetScroller (int tag, EScroll type, double dx, double dy)
{
	...
	// [BB] The client needs to check the values it got from the server.
	if ( NETWORK_InClientMode () )
	{
		// Check to make sure what we've read in is valid.
		if (( type != EScroll::sc_floor ) && ( type != EScroll::sc_ceiling ) &&
			( type != EScroll::sc_carry ) && ( type != EScroll::sc_carry_ceiling ))
		{
			/*client_PrintWarning*/ Printf ("client_SetScroller: Unknown type: %d!\n", static_cast<int> ( type ));
			return;
		}
	}
...
void P_CreateScroller(EScroll type, double dx, double dy, int control, int affectee, int accel, EScrollPos scrollpos)
{
	...
	{
		if (( type != EScroll::sc_floor ) && ( type != EScroll::sc_ceiling ) &&
			( type != EScroll::sc_carry ) && ( type != EScroll::sc_carry_ceiling ) && ( type != EScroll::sc_side ))
		{
			/*client_PrintWarning*/ Printf ("client_DoScroller: Unknown type: %d!\n", static_cast<int> ( type ));
			return;
		}

		if (type == EScroll::sc_side)
		{
			if (( affectee < 0 ) || ( affectee >= numsides ))
			{
				/*client_PrintWarning*/ Printf ("client_DoScroller: Invalid side ID: %ld!\n", affectee);
				return;
			}
		}
		else if (( affectee < 0 ) || ( affectee >= numsectors ))
		{
			/*client_PrintWarning*/ Printf ("client_DoScroller: Invalid sector ID: %ld!\n", affectee);
			return;
		}
	}
